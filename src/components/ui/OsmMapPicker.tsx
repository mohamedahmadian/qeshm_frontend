...
function projectPinHtml(marker: MapOverlayMarker) {
  ...
  const pulseStrong = marker.pulseStrong
    ? " eskan-project-pin-pulse-strong"
    : "";
  const spidered = marker.spidered ? " eskan-project-pin-spidered" : "";
  const color = isProjectColor(marker.color)
    ? projectColor(marker.color)
    : "#2ebdb6";
...
function overlayFitLatLngs(
  overlays: MapOverlays | null,
  extra?: L.LatLng | null,
) {
  if (!overlays) return [];
  if (overlays.fitPoints?.length) {
    const points = overlays.fitPoints.map((point) =>
...
/** Levels the live board can zoom out past the island frame, with room to pan. */
const LIVE_BOARD_UNZOOM_LEVELS = 6;

/** At a tight maxBounds the view snaps back as soon as it is larger than the island. */
function boundsWithPanRoom(map: L.Map, bounds: L.LatLngBounds, zoom: number) {
  const size = map.getSize();
  if (!zoom || size.x < 80 || size.y < 80) return bounds.pad(1.5);
  const sw = map.project(bounds.getSouthWest(), zoom);
  const ne = map.project(bounds.getNorthEast(), zoom);
  const width = Math.abs(ne.x - sw.x);
  const height = Math.abs(ne.y - sw.y);
  if (width < 1 || height < 1) return bounds.pad(1.5);
  const scale = Math.max(1, (size.x * 3) / width, (size.y * 2.2) / height);
  return bounds.pad((scale - 1) / 2);
}
...
function limitMapBounds(
  ...
  const fitted =
    overviewZoom && overviewZoom > 0 ? overviewZoom : map.getZoom();
  const minZoom = Math.max(3, Math.floor(fitted) - LIVE_BOARD_UNZOOM_LEVELS);
  map.setMinZoom(minZoom);
  map.setMaxBounds(boundsWithPanRoom(map, bounds, minZoom));
  map.options.maxBoundsViscosity = 0.15;
}

function panMapFromEdgeTap(map: L.Map, event: L.LeafletMouseEvent) {
  const size = map.getSize();
  const point = event.containerPoint;
  const margin = Math.min(88, Math.round(Math.min(size.x, size.y) * 0.22));
  if (margin < 24) return false;
  const nearLeft = point.x <= margin;
  const nearRight = point.x >= size.x - margin;
  const nearTop = point.y <= margin;
  const nearBottom = point.y >= size.y - margin;
  if (!nearLeft && !nearRight && !nearTop && !nearBottom) return false;
  const stepX = Math.round(size.x * 0.42);
  const stepY = Math.round(size.y * 0.42);
  map.panBy(
    [
      nearLeft ? -stepX : nearRight ? stepX : 0,
      nearTop ? -stepY : nearBottom ? stepY : 0,
    ],
    { animate: true },
  );
  return true;
...
function stageMapView(map: L.Map, target: L.LatLng, targetZoom: number) {
  ...
  const endZoom = Math.max(
    map.getMinZoom(),
    Math.min(map.getMaxZoom(), targetZoom),
  );
  if (start.distanceTo(target) < 4 && Math.abs(startZoom - endZoom) < 0.15)
    return;
...
function paintOverlayContents(
  ...
  for (const polygon of overlays.polygons ?? []) {
    ...
    const color = isProjectColor(polygon.color)
      ? projectColor(polygon.color)
      : "#2ebdb6";
...
export function OsmMapPicker({
  ...
  const overviewViewRef = useRef<{ center: L.LatLng; zoom: number } | null>(
    null,
  );
  ...
  function placeMarker(map: L.Map, latlng: L.LatLng, draggable: boolean) {
    ...
    markerRef.current = L.marker(latlng, { icon: pinIcon, draggable }).addTo(
      map,
    );
    if (draggable) {
  ...
  useEffect(() => {
    ...
    if (start) {
      ...
      currentOverlays?.fit &&
      overlayFitLatLngs(currentOverlays).length
    ) {
      ...
      map.fitBounds(toLeafletBounds(focus.bounds), {
        padding: [56, 56],
      ...
      map.fitBounds(toLeafletBounds(IRAN_BOUNDS), {
        padding: [28, 28],
        maxZoom: 6,
      });
    }
    ...
    if (canEdit) {
      map.on("click", (event: L.LeafletMouseEvent) => {
        ...
        onChangeRef.current(
          formatCoord(event.latlng.lat),
          formatCoord(event.latlng.lng),
        );
      });
    } else {
      map.on("click", (event: L.LeafletMouseEvent) => {
        ...
        if (
          ...
          origin.closest(
            ".leaflet-marker-icon, .leaflet-interactive, .leaflet-tooltip, .leaflet-control",
          )
        ) {
          return;
        }
        if (onMapClickRef.current && panMapFromEdgeTap(map, event)) {
          userMoved = true;
          return;
        }
    ...
    map.on("zoomstart", (event) => {
      if ("originalEvent" in event && event.originalEvent) userMoved = true;
    });
    const observer = new ResizeObserver(() => {
      map.invalidateSize({ animate: false });
      const size = map.getSize();
      if (size.x < 80 || size.y < 80 || selectedZoomIdRef.current || userMoved)
        return;
      const grew =
        !sized ||
        Math.abs(size.x - fittedWidth) > 48 ||
        Math.abs(size.y - fittedHeight) > 48;
      if (!grew) return;
      sized = true;
      fittedWidth = size.x;
      fittedHeight = size.y;
      ...
      const bounds = toLeafletBounds(currentBounds);
      const loose = zoomOnSelectedRef.current;
      applyBoundsView(map, bounds);
      overviewViewRef.current = {
        center: map.getCenter(),
        zoom: map.getZoom(),
      };
      limitMapBounds(map, bounds, loose, map.getZoom());
    });
  ...
  useEffect(() => {
    ...
    if (maxBounds) {
      ...
      limitMapBounds(map, bounds, loose, overviewViewRef.current?.zoom);
      return;
    }
    map.setMaxBounds(undefined as unknown as L.LatLngBounds);
    if (focus?.bounds) {
  ...
  useEffect(() => {
    ...
    if (!selectedId) {
      ...
      if (overview) {
        ...
        stageMapView(
          map,
          bounds.getCenter(),
          map.getBoundsZoom(bounds, false, L.point(20, 20)),
        );
      }
      return;
    }
    ...
    const targetZoom = Math.min(
      overviewZoom + SELECT_ZOOM_LEVELS,
      SELECT_MAX_ZOOM,
    );
  ...
  useEffect(() => {
    ...
    function resizeAndFit() {
      ...
      const here = parseLatLng(latitude, longitude);
      overlayFitKeyRef.current = overlayFitKey(
        overlayFitLatLngs(current, here),
      );
      fitOverlayBounds(map, current, here);
    }
  ...
  useEffect(() => {
    ...
    const marker = overlaysRef.current?.markers.find(
      (item) => item.id === keepInView.id,
    );
    const polygon = overlaysRef.current?.polygons?.find(
      (item) => item.id === keepInView.id,
    );
    ...
    const timer = window.setTimeout(() => {
      if (marker) {
        map.panInside(L.latLng(marker.lat, marker.lng), {
          ...
          paddingBottomRight: [
            keepInView.padding.right,
            keepInView.padding.bottom,
          ],
          animate: true,
        });
        return;
      ...
      if (bounds.isValid())
        map.panInside(bounds.getCenter(), {
          paddingTopLeft: [keepInView.padding.left, keepInView.padding.top],
          paddingBottomRight: [
            keepInView.padding.right,
            keepInView.padding.bottom,
      ...
      frame = window.requestAnimationFrame(() => {
        ...
        const latlng =
          "lat" in selected && "lng" in selected
            ...
            : L.latLngBounds(
                selected.latlngs.map((point) => L.latLng(point.lat, point.lng)),
              ).getCenter();
  ...
  function applyPosition(lat: number, lng: number, fromGeo: boolean) {
    ...
    if (map) {
      ...
      map.setView(
        latlng,
        Math.min(map.getZoom() || pinZoomRef.current, pinZoomRef.current),
      );
    }
    return "ok" as const;
  }
        ...
        <div className="flex flex-wrap gap-2">
          {showMapToggle ? (
            ...
            >
              ...
              {open
                ? t("accommodations.hideMap")
                : t("accommodations.pickFromMap")}
            </Button>
          ) : null}
          {showGeoButton ? (
            <Button
